...
Future<bool> showPilihKendaraanDialog(
    BuildContext context, int idPenugasan) async {
...
class _PilihKendaraanDialogState extends State<_PilihKendaraanDialog> {
  ...
  Future<void> _confirmAndSubmit(KendaraanModel kendaraan) async {
    ...
    try {
      await PenugasanService.pilihKendaraan(
          idPenugasan: widget.idPenugasan, idKendaraan: kendaraan.id);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              '${kendaraan.nopol} dipilih. Perjalanan dimulai, selamat bertugas!'),
      ...
      child: ConstrainedBox(
        ...
        child: FutureBuilder<List<KendaraanModel>>(
          ...
          builder: (context, snapshot) {
            ...
            final selectedList =
                list.where((k) => k.id == _selectedId).toList();
            final selectedKendaraan =
                selectedList.isEmpty ? null : selectedList.first;
                ...
                Flexible(
                  child: Builder(builder: (context) {
                    ...
                    return ListView(
                      ...
                      children: [
                        const Text('Pilih Kendaraan Tersedia *',
                            style: TextStyle(
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: (_submitting || selectedKendaraan == null)
                              ? null
                              : () => _confirmAndSubmit(selectedKendaraan),
                          icon: _submitting
                              ? const SizedBox(
                                  width: 16,
                                  height: 16,
...
class _KendaraanCard extends StatelessWidget {
  ...
  const _KendaraanCard(
      {required this.kendaraan, required this.selected, required this.onTap});
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                  color: selected ? AppColors.success : const Color(0xFFE8EDF2),
                  width: selected ? 2 : 1),
            ),
            child: Row(
              children: [
